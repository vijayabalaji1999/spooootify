...
const ShowSongs = ({ songs, handleSongClick, lastSongRef, favourites }) => {
  const { bannerSong, setSongList } = useContext(GlobalContext);
  ...
  const handleDragEnd = (event) => {
    ...
    if (active.id !== over.id) {
      ...
      console.log(bannerSong, '==bannerSong');

      if (bannerSong) {
        tempSongs.movie_link = bannerSong.movie_link;
        tempSongs.image_url = bannerSong.movie_image;
        tempSongs.movie_name = bannerSong.movie_name;
      }
