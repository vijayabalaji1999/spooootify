...
const SongBanner = () => {
  ...
  useEffect(() => {
    if (songList) {
      const activeIndex = songList.songs.findIndex((song) => song.active);
      const result =
        activeIndex !== -1
          ? { ...songList.songs[activeIndex], index: activeIndex }
          : null;
  ...
  useEffect(() => {
    ...
    const nextSong = () => {
      ...
      const nextSongIndex = (currentSongIndex + 1) % songList.songs.length;
      const nextSongLink = songList.songs[nextSongIndex].link;
      audio.pause();
      audio.src = nextSongLink;
      setCurrentSong({
        ...songList.songs[nextSongIndex],
        index: nextSongIndex,
      });
      audio.src = nextSongLink;
      audio.load();
  ...
  const handleNext = () => {
    if (!currentSong || !songList?.songs) return;
    const nextSongIndex = (currentSong.index + 1) % songList.songs.length;
    audio.pause();
    audio.src = songList.songs[nextSongIndex].link;
    setCurrentSong({
      ...songList.songs[nextSongIndex],
      index: nextSongIndex,
    });
  ...
  const handlePrevious = () => {
    if (!currentSong || !songList?.songs) return;
    const prevSongIndex =
      (currentSong.index - 1 + songList.songs.length) % songList.songs.length;
    audio.pause();
    audio.src = songList.songs[prevSongIndex].link;
    setCurrentSong({
      ...songList.songs[prevSongIndex],
      index: prevSongIndex,
    });
    audio.load();
    audio.play();
  ...
  const gotoSongList = () => {
    ...
    router.push(`/song/${songList?.movie_link}`);
  };
    ...
    <>
      <div className='song-banner' onClick={handleBannerClick}>
        <div className='song-info'>
          <img
            src={songList?.image_url || 'default-album-art.jpg'}
            alt='Album art'
      ...
      {showMobilePlayer && (
        <div className='mobile-player'>
          ...
          <div className='mobile-player-content'>
            ...
            <div onClick={gotoSongList} className='mobile-song-info'>
              ...
              <p className='mobile-song-artist'>{songList?.movie_name}</p>
            </div>
